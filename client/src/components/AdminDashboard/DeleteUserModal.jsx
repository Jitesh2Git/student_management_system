...
import { deleteUserByAdmin } from "../../store/slices/adminSlice";
...
const DeleteUserModal = ({ user, onCancel }) => {
  ...
  const handleDelete = async () => {
    const resultAction = await dispatch(deleteUserByAdmin(user._id));
    ...
    if (deleteUserByAdmin.fulfilled.match(resultAction)) {
      ...
      showToast({
        type: "error",
        message: resultAction.payload || "Failed to delete user",
      });
    }
  };
